...
### TJCE case law for Claude, Cursor and AI agents

Search case law from the Ceará state court (TJCE) straight from Claude, ChatGPT or your own agent, covering cível, consumidor e imobiliário. Every ruling comes with the judging body, rapporteur, date, the exact snippet that matched your query and the link to the official site. The same connection reaches 16 other Brazilian courts. Free, no login, hosted by the platform.

- ⚖️ **TJCE** plus 16 other Brazilian courts on the same connection
...
**Manual** (if the deeplink does not open): [claude.ai/customize/connectors](https://claude.ai/customize/connectors?surface=cowork) → **+** → **Add custom connector** → paste **Name** `Jurisprudência TJCE` and **URL** `https://api.mcp.ai/p_tjce`.

### Cursor

[➕ Install Jurisprudência TJCE in Cursor](cursor://anysphere.cursor-deeplink/mcp/install?name=tjce&config=eyJ1cmwiOiJodHRwczovL2FwaS5tY3AuYWkvcF90amNlIn0=)

### VS Code (Copilot Chat)

[➕ Install Jurisprudência TJCE in VS Code](vscode:mcp/install?name=tjce&config=%7B%22type%22%3A%22http%22%2C%22url%22%3A%22https%3A%2F%2Fapi.mcp.ai%2Fp_tjce%22%7D)
...
```
Recent TJCE rulings on consumer damages
Compare TJCE and the Superior Court on limitation periods
...
MIT — see [LICENSE](LICENSE). The MCP server at `api.mcp.ai/p_tjce` is proprietary (hosted); this repo (manifests, docs, skills) is MIT.
